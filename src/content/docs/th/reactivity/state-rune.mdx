...
`$state(value)` สร้าง reactive state ที่คุณอ่านและ reassign เหมือนตัวแปรปกติ สำหรับ **object และ array จะได้ deep proxy** ดังนั้น *mutation* (`arr.push`, `obj.x = …`) เป็น reactive — ไม่ต้องสร้าง object ใหม่แบบ React ใช้ **`$state.raw`** กับค่าที่คุณแค่ replace ทั้งก้อน (reactive ตอน reassign ไม่ใช่ตอน mutate) เพื่อเลี่ยง overhead ของ proxy และใช้ **`$state.snapshot(x)`** เพื่อเอา plain copy ที่ไม่ใช่ proxy ไป log, clone หรือส่งให้ code ภายนอก
</Callout>
...
export const quizState = [
  {
    ...
    options: [
      ...
      'reassign หรือ mutate ได้ตามปกติ — `count++` หรือ `obj.x = 1` สำหรับ $state object',
      'dispatch action',
      'เรียก $state.update()',
    ],
    answer: 1,
